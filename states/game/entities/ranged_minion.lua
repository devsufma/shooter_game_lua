...
function ent:load(x,y)
	self:setPos(x, y)
	self.imageRight = love.graphics.newImage("Images/Sprites/ranged_minion_right.png")
	self.imageLeft = love.graphics.newImage("Images/Sprites/ranged_minion_left.png")
	self.w = 38.25
	self.h = 30.75
...
function ent:draw()
	...
	love.graphics.rectangle("fill", self.x, self.y-4, self.health/5*30, 4)
end
...
function ent:update(dt)
	...
	if deltaTime > self.atkspeed then
		local bullet = ents.Create("enemy_bullet", self.x, self.y, "none", "ranged_minion")
		bullet:setDestiny()
		deltaTime = 0
	end
