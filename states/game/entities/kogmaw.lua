...
function ent:load(x,y)
	self:setPos(x, y)
	self.imageRight = love.graphics.newImage("Images/Sprites/kogmaw-right.png")
	self.imageLeft = love.graphics.newImage("Images/Sprites/kogmaw-left.png")
	self.w = 28.2
	self.h = 34.2
...
function ent:draw()
	...
	love.graphics.draw(self.image, self.x, self.y, 0, 0.6, 0.6, 0)

	love.graphics.setColor(255, 0, 0, 255)
	love.graphics.rectangle("fill", self.x, self.y-4, self.health/20*30, 4)
end
...
function ent:update(dt)
	...
	if deltaTime > self.atkspeed then
		local bullet = ents.Create("enemy_bullet", self.x, self.y, "none", "kogmaw")
		bullet:setDestiny()
		deltaTime = 0
	end
...
function ent:returnBulletspd()
	return self.bulletspd
end
