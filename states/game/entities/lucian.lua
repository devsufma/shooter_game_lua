...
function ent:load(x,y)
	self:setPos(x, y)
	self.imageRight = love.graphics.newImage("Images/Sprites/lucian-right.png")
	self.imageLeft = love.graphics.newImage("Images/Sprites/lucian-left.png")
	self.w = 47.25
	self.h = 39
...
function ent:draw()
	...
	love.graphics.rectangle("fill", self.x, self.y-4, self.health/50*30, 4)
end
...
function ent:update(dt)
	...
	if deltaTime > self.atkspeed then
		local bullet = ents.Create("enemy_bullet", self.x, self.y, "none", "lucian")
		bullet:setDestiny()
		deltaTime = 0
	end
